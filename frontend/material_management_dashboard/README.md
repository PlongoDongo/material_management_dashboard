...
| `data/repository.py` | **Die Grenze.** Ruft den Client, bildet API-Felder auf Tabellenspalten ab, cached, fängt Ausfälle ab. Der Rest der App ruft nur `get_materials()` — und nach einem Schreibvorgang `invalidate()`. |
